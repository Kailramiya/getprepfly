...
export async function POST(req: NextRequest) {
  ...
  // Case 1: Student already has an account → link directly
  const existingUser = await db.user.findUnique({ where: { email: normalizedEmail } });
  if (existingUser) {
    if (existingUser.centreId === centreId) {
      return NextResponse.json({ success: false, error: "This student is already in your centre" }, { status: 409 });
    }
    ...
    return NextResponse.json({
      success: true,
      data: { status: "linked", message: `${existingUser.name} has been added to your centre.` },
    });
  }
