...
export default function AdminAnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Centre Analytics</h1>
        <p className="text-gray-500">{user?.centreName} — Performance Overview</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          ...
          { label: "Active Students", value: stats?.activeStudents || 0, icon: TrendingUp, color: "text-green-500", bg: "bg-green-50" },
          { label: "Total Practice", value: stats?.totalAttempts || 0, icon: BarChart3, color: "text-teal-500", bg: "bg-teal-50" },
          { label: "Mock Tests Taken", value: stats?.totalMockTests || 0, icon: Clock, color: "text-purple-500", bg: "bg-purple-50" },
        ].map((stat) => (
          <Card key={stat.label}>
            ...
            <div className="flex items-center gap-6">
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">{(stats?.totalStudents || 0) - (stats?.vipStudents || 0)}</span>
                </div>
            ...
            <div className="flex items-center gap-6">
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">{stats?.activeStudents || 0}</span>
                </div>
                <div className="h-3 rounded-full bg-gray-200">
                  <div
                    className="h-full rounded-full bg-green-500"
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">{stats?.inactiveStudents || 0}</span>
                </div>
                <div className="h-3 rounded-full bg-gray-200">
                  <div
      ...
      <Card>
        ...
        <CardContent>
          {students.length === 0 ? (
            <p className="py-8 text-center text-sm text-gray-400">No students yet</p>
          ) : (
            <div className="space-y-2">
              {students
                ...
                .map((student: any, i: number) => (
                  <div key={student.id} className="flex items-center justify-between rounded-lg bg-gray-50 p-3">
