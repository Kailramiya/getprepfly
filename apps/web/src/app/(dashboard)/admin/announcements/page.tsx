  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Announcements</h1>
          <p className="text-gray-500 dark:text-slate-400">Send messages to all your students</p>
        </div>
        ...
        <Card className="border-indigo-200 bg-indigo-50 dark:border-indigo-900 dark:bg-indigo-950/50">
          <CardContent className="p-5">
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <textarea
                className="min-h-[80px] w-full rounded-lg border border-gray-300 p-3 text-sm focus:border-indigo-500 focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500"
      ...
      <Card>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-16 text-center"><Megaphone className="mx-auto h-12 w-12 text-gray-300" /><p className="mt-3 text-gray-500 dark:text-slate-400">No announcements yet.</p></div>
          ) : (
              ...
              {list.map(a => (
                <div key={a.id} className="flex items-start justify-between gap-4 p-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600 dark:text-slate-300">{a.message}</p>
                    <p className="mt-1 text-xs text-gray-400 dark:text-slate-500">{new Date(a.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
                  </div>
                  {!a.isGlobal && (
                    <button onClick={() => handleDelete(a.id)} className="shrink-0 text-gray-400 hover:text-red-600 dark:text-slate-500 dark:hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
                  )}
                </div>
              ))}
            </div>
